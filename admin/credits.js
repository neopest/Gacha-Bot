...
module.exports = {
    ...
    execute: async (message, args) => {
        ...
        if (!targetUserId || isNaN(creditAmount) || creditAmount < 0) {
            message.channel.send("❌ Invalid usage. Correct format: `addcredits @User <amount>`.");
            return;
        }
        ...
        const updatedCredits = data.users[targetUserId].credits + creditAmount;
        data.users[targetUserId].credits = updatedCredits;

        DataManager.saveData(data);

        message.channel.send(`✅ Added ${creditAmount} Credits to <@${targetUserId}>. New balance: ${updatedCredits}`);
        await targetUser.send(`✅ Your Credits have been increased by ${creditAmount}. New balance: ${updatedCredits}`);
    }
};
